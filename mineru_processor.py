...
class MineruProcessor:
    ...
    def _get_pdf_file_paths(self, root_dir: str) -> list:
        print(f"正在从 '{root_dir}' 目录中查找所有 .pdf 文件...")
        file_paths = glob.glob(os.path.join(root_dir, "**", "*.pdf"), recursive=True)
        print(f"成功找到 {len(file_paths)} 个PDF文件。")
        return file_paths
    ...
    def _get_upload_urls(self, file_paths: list) -> tuple:
        print("\n--- 步骤 1: 正在向Mineru API申请文件上传URL ---")
        url = f"{self.base_url}/file-urls/batch"
        files_data = [{"name": os.path.basename(p), "is_ocr": True, "data_id": p} for p in file_paths]
        payload = {"enable_formula": True, "language": "ch", "enable_table": True, "files": files_data}
        ...
        if result.get("code") == 0:
            ...
            print(f"成功获取上传URL。Batch ID: {batch_id}")
    ...
    def _upload_files(self, local_files: list, upload_urls: list):
        ...
        if len(local_files) != len(upload_urls):
            print(f"[警告] 文件列表数量({len(local_files)})与URL列表数量({len(upload_urls)})不匹配，跳过上传。")
            return
        upload_count = 0
        for local_path, upload_url in zip(local_files, upload_urls):
            try:
                ...
                print(f"  - {os.path.basename(local_path)} ... 上传失败! 错误: {e}")
        if upload_count == len(local_files):
            print("当前批次的所有文件均已成功上传。")


    def _poll_for_results(self, batch_id: str) -> list:
        print("\n--- 步骤 3: 正在等待解析结果 (此过程可能需要较长时间) ---")
        ...
        while True:
            ...
            if result.get("code") == 0:
                ...
                print(f"  - 批次总体状态: {top_level_status} (已等待 {int(time.time() - start_time)} 秒)")
                if top_level_status == "SUCCESS":
                    results_list = data.get("extract_result")
                    if isinstance(results_list, list):
                        pending_files = [item.get('file_name') for item in results_list if item.get("state") in ['pending', 'waiting-file', 'processing']]
                        if not pending_files:
                            print("  - 所有文件的状态均已完成！解析任务成功结束！")
                            return results_list
                        else:
                            done_count = len(results_list) - len(pending_files)
                            print(f"  - 批次处理中: {done_count}/{len(results_list)} 个文件已完成。仍在等待 {len(pending_files)} 个文件... ({pending_files[0]} 等)")
                    else:
                        print(f"  - 批次状态为SUCCESS，但尚未返回文件列表，继续等待...")
                elif top_level_status == "FAILED":
                    raise Exception("解析任务失败。")
            else:
                print(f"  - 任务仍在处理中... (API code: {result.get('code')}, msg: {result.get('msg')})")
    ...
    def _save_results(self, file_results_list: list, output_dir: str, source_dir: str):
        ...
        for file_result in file_results_list:
            ...
            if state != 'done' or not zip_url:
                print(f"  - [警告] 文件 {base_name} 处理未成功或没有zip链接，状态: '{state}'，已跳过。")
                continue
            try:
                ...
                with zipfile.ZipFile(zip_in_memory, 'r') as zf:
                    ...
                    print(f"  - 正在从ZIP中提取: {markdown_filename}")
                    markdown_content = zf.read(markdown_filename).decode('utf-8')
                    with open(output_filepath, 'w', encoding='utf-8') as f:
                        f.write(markdown_content)
                    print(f"  - 已成功保存至: {output_filepath}")
            except Exception as e:
                print(f"  - [错误] 处理 {base_name} 的结果时发生意外: {e}")
        print("当前批次的所有结果均已处理完毕。")
...
if __name__ == "__main__":
    ...
    if not MINERU_API_KEY:
        print("错误：请确保 .env 文件中已设置 MINERU_API_KEY。")
    else:
        # 2. 定义输入和输出目录
        pdf_source_directory = "dataset"
        markdown_output_directory = "output_markdown"

        # 3. 创建 MineruProcessor 实例
        # 你可以在这里自定义参数，例如 processor = MineruProcessor(api_key=MINERU_API_KEY, batch_size=20)
