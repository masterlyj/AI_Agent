...
if TYPE_CHECKING:
    from .base import BaseKVStorage, QueryParam
...
class Tokenizer:
    ...
    def __init__(self, model_name: str, tokenizer: TokenizerInterface):
        """
        使用分词器模型名称和分词器实例初始化分词器。

        参数：
            model_name：分词器的关联模型名称。
            tokenizer：实现TokenizerInterface的类的实例。
        """
...
class TiktokenTokenizer(Tokenizer):
    ...
    def __init__(self, model_name: str = "gpt-4o-mini"):
        """
        使用指定的模型名称初始化TiktokenTokenizer。

        参数：
            model_name：要使用的tiktoken分词器的模型名称。默认为“gpt-4o-mini”。

        异常：
            ImportError：如果未安装tiktoken。
            ValueError：如果model_name无效。
        """
        try:
            import tiktoken
        except ImportError:
