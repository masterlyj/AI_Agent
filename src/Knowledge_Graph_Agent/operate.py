...
async def extract_entities(
    ...
    processed_chunks = 0  # 已处理的chunk计数器
    total_chunks = len(ordered_chunks)

    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        """
        处理单个chunk内容并返回（实体，关系）
        Args:
            chunk_key_dp (tuple[str, TextChunkSchema]): 
                ("chunk-xxxxxx", {"tokens": int, "content": str, ...})
        Returns:
            tuple: (maybe_nodes, maybe_edges) 抽取的实体和关系
        """
        nonlocal processed_chunks
        chunk_key = chunk_key_dp[0]
        chunk_dp = chunk_key_dp[1]
        content = chunk_dp["content"]
        # 从chunk数据中获取文件路径（注：有可能没有）
        ...
        if entity_extract_max_gleaning > 0:
            ...
            # 处理补漏（gleaning）结果，file_path用于后续引用
            ...
            # 合并结果。逻辑为保留描述信息更丰富的那一份（按description长度），entity和edge分别做一遍
    ...
    # 获取并发控制Semaphore，读取最大异步数（如未配置默认4）
    ...
    async def _process_with_semaphore(chunk):
        async with semaphore:
            try:
                ...
                chunk_id = chunk[0]  # chunk_id，用于异常前缀
    ...
    # 所有任务均成功则直接返回所有chunk的抽取结果（供后续merge）
...
async def kg_query(
    ...
    参数：
        ...
        text_chunks_db：文本片段存储
        query_param：查询参数
        global_config：全局配置
        hashing_kv：缓存存储
        system_prompt：系统提示词
        chunks_vdb：文档片段向量数据库

    返回：
        QueryResult：统一的查询结果对象，包含：
            - content：非流式响应文本内容
            - response_iterator：流式响应迭代器
            - raw_data：完整的结构化数据（包括引用和元数据）
            - is_streaming：是否为流式结果

        根据不同的query_param设置，会填充不同的字段：
        - only_need_context=True：content包含上下文字符串
        - only_need_prompt=True：content包含完整的提示词
        - stream=True：response_iterator包含流式响应，raw_data包含完整数据
        - 默认：content包含大语言模型响应文本，raw_data包含完整数据
    """
    if not query:
